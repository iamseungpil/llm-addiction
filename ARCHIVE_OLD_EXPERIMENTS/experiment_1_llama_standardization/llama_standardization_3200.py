#!/usr/bin/env python3
"""
LLaMA Slot Machine Experiment (3,200 games to match GPT/Gemini format)
- Mirrors gemini_experiment protocol exactly
- Same game mechanics, prompt structure, and parsing logic
- Results saved under /data/llm_addiction/experiment_1_llama_standardization/
- 3,200 total experiments (64 conditions × 50 repetitions)
"""
...
class LlamaStandardizationExperiment:
    def __init__(self, resume_from_file: Optional[str] = None, resume_from_experiment: Optional[int] = None):
        ...
        self.log_file = self.logs_dir / f'llama_standardization_{timestamp}.log'
    ...
    def load_models(self):
        ...
        print(f"Loading LLaMA Model on GPU {gpu_id} (appears as cuda:0 to PyTorch)")
        print("="*80)
    ...
    def _get_model_response(self, prompt: str) -> str:
        ...
        for attempt in range(1, max_retries + 1):
            try:
                ...
                self.log(f"LLaMA model error (attempt {attempt}/{max_retries}): {exc}")
    ...
    def run_experiment(self):
        ...
        if self.resume_from_file:
            ...
            output_file = self.results_dir / f'llama_standardization_{timestamp}.json'

        print(f"Starting LLaMA standardization experiment: {len(conditions)} conditions × {repetitions} repetitions")
        print(f"Total simulations: {total_experiments}")
        if self.resume_from_experiment:
            print(f"Resuming from experiment {self.resume_from_experiment}")
        print(f"Results will be saved to: {output_file}")

        pbar = tqdm(total=total_experiments, initial=self.current_experiment, desc="Running LLaMA experiments")
        ...
        try:
            ...
            avg_rounds = sum(r['total_rounds'] for r in self.results) / len(self.results) if self.results else 0
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Run LLaMA standardization slot machine experiment")
