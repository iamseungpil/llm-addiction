...
class GemmaAdditionExperiment:
    """Gemma 3,200 games addition experiment"""
    ...
    def load_models(self):
        ...
        # Load LLaMA model instead (Gemma has compatibility issues)
    ...
    def extract_hidden_states(self, prompt: str) -> Dict:
        ...
        with torch.no_grad():
            ...
            # Gemma-2-9B has 42 layers, extract from key layers
            for layer_idx in [10, 20, 30, 41]:  # Sample key layers
                layer_hidden = hidden_states[layer_idx]
                # Use last token hidden state
                layer_features = layer_hidden[0, -1, :].cpu().numpy().tolist()
                features_dict[f'layer_{layer_idx}_hidden'] = layer_features
    ...
    def run_single_game(self, bet_type: str, prompt_combo: str,
        ...
        while not game.is_finished and game.round < self.max_rounds:
            ...
            features_dict = self.extract_hidden_states(prompt)

            # Generate response
            response, decision, bet_amount = self.generate_response(prompt)

            # Store round data with hidden states
            round_data = {
                'round': game.round + 1,
                'prompt': prompt,
                'response': response,
                ...
                **features_dict  # Include all hidden states
            }
    ...
    def run_experiment(self):
        ...
        for bet_type, prompt_combo in tqdm(
            ...
            for rep in range(num_repetitions):
                ...
                if experiment_id % 100 == 0:
                    ...
                    intermediate_file = self.results_dir / f'gemma_add_intermediate_{experiment_id}_{timestamp}.json'
        ...
        # Save final results
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        output_file = self.results_dir / f'gemma_addition_complete_{timestamp}.json'
        ...
        print("🎯 Gemma Addition Experiment Complete!")
