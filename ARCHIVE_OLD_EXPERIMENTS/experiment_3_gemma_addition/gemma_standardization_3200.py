#!/usr/bin/env python3
"""
Experiment 3: Gemma 3,200-game Standardization Experiment
...
class GemmaMultiRoundExperiment:
    def __init__(self):
        self.device = 'cuda:0'
        ...
        self.results_dir = Path('/data/llm_addiction/experiment_3_gemma_addition')
    ...
    def _get_model_response(self, prompt: str) -> str:
        ...
        for attempt in range(1, max_retries + 1):
            try:
                ...
                self.log(f"Gemma generation error (attempt {attempt}/{max_retries}): {e}")
    ...
    def run_experiment(self):
        ...
        self.log("🚀 Starting Gemma 3,200-game Standardization Experiment")
        self.log("="*80)
    ...
    def save_intermediate_results(self, count: int):
        ...
        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        filename = self.results_dir / f'gemma_intermediate_{count}_{timestamp}.json'
    ...
    def save_final_results(self):
        """Save final results"""
        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        filename = self.results_dir / f'gemma_final_{timestamp}.json'
