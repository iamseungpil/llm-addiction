#!/usr/bin/env python3
"""
Experiment 4: Multi-round Patching (441 causal features)
Extended patching for full game duration:
- Use verified 441 causal features from previous experiments
- Apply persistent patching throughout entire game
- Test behavioral changes across multiple rounds
"""
...
class MultiroundPatchingExperiment:
    ...
    def generate_with_persistent_patching(self, prompt: str, feature_info: Dict, patch_value: float):
        ...
        def patch_hook(module, input, output):
            ...
            if hasattr(output, 'shape') and len(output.shape) >= 3:
                if layer >= 25:  # Use SAE for L25-30
                    ...
                    if sae is not None:
                        # More complex SAE-based patching
        ...
        if layer < len(self.model.model.layers):
            ...
            # Fallback for layer indexing issues
            handle = None
    ...
    def run_experiment(self):
        ...
        print("🚀 Starting Multi-round Patching Experiment (441 causal features)")
        print("="*80)
        ...
        for i, feature_info in enumerate(tqdm(high_effect_features, desc="Testing features")):
            ...
            try:
                ...
                if (i + 1) % 10 == 0:
                    ...
                    intermediate_file = self.results_dir / f'multiround_intermediate_{i+1}_{timestamp}.json'

                    save_data = {
                        'timestamp': timestamp,
                        'progress': f'{i+1}/{len(high_effect_features)}',
