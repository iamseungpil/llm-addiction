#!/usr/bin/env python3
"""
Pilot Test with Claude: Check if LLMs reference previous goals
...
def main():
    ...
    # Save raw results
    results_file = pilot_dir / f'pilot_claude_raw_results_{timestamp}.json'
    ...
    # Save analysis
    analysis_file = pilot_dir / f'pilot_claude_analysis_{timestamp}.json'
    ...
    report_file = pilot_dir / f'pilot_claude_report_{timestamp}.txt'
    with open(report_file, 'w', encoding='utf-8') as f:
        f.write("="*80 + "\n")
        f.write("PILOT TEST REPORT: Goal Reference Pattern Analysis (Claude)\n")
        f.write("="*80 + "\n\n")
        f.write(f"Date: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
        f.write(f"Model: Claude 3.5 Haiku\n")
