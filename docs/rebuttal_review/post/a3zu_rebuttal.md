...
You are right that no independent annotator reviewed the flagged sentences. The purpose of this analysis is not to classify a sentence or a model clinically, but to compare on one consistent rule how often language patterns defined in prior gambling research appear across conditions.
...
We also checked whether the phenomenon is produced by the extraction rule at all. In the two runs where the game environment stores the goal itself, nothing is read out of the reasoning text and no extraction takes place. In that parser-independent measurement, goal escalation still occurs in 24.6% and 30.6% of games in the goal conditions. Raising a target after meeting it is therefore not an artefact of the text parser. The revision separates the text-extracted and environment-recorded measurements and states which measurement each reported figure comes from, instead of pooling the two.
...
We did not run a human control on this slot machine under the same autonomy conditions, and the study does not measure whether a model's rates match human clinical rates. Human gambling research was used to construct situations known to elicit risky choice and persistence and to decide which behaviours and which language to measure; our question is how a model's behaviour changes, relatively, with betting freedom and with the prompt. We therefore do not compare our rates to human rates or claim that people would show an effect of the same size. Whether humans and models move in the same direction and by the same amount on an identical task needs a matched human study, and the revision will say plainly that the human literature grounds the task and the measurement design while our results are within-model behavioural contrasts.
...
A single worked example changed the betting policy that followed, and the clearest difference in bankruptcy appeared in Gemini's variable condition. To test this we gave four API models three arms under BASE: no demonstration, a cautious example, and an escalating example. Each arm ran 100 games at a cap of \$70, with the instructions, the game description and the seeds held identical, so that only the demonstration differed. The cautious example holds a \$20 wager across rounds and then stops; the escalating example raises its wager from \$20 to \$40 after the first loss. Both were specified before the experiment and matched on length, on the number of rounds shown and on both ending in a stop, and neither states a rule or a recommendation, so what differs between them is the trajectory rather than any advice.

| model, condition | no demo | cautious | escalating | escalating − cautious | mean stake per played round |
|---|---|---|---|---|---|
...
Your hypothesis is therefore supported in part. A single example can calibrate what follows, but a direction-specific effect on bankruptcy is at present confined to Gemini's variable condition and we will not generalise it; whether the other cells reflect insensitivity to examples or a participation floor is not separable from these data. We are grateful for a proposal that let us test prompt sensitivity as imitation of a concrete behavioural strategy.
