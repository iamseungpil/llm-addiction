#!/usr/bin/env python3
"""
Gemma Irrationality Metrics Analysis
...
def load_gemma_data():
    """Load Gemma experimental data"""
    print("Loading Gemma data...")
    with open('/data/llm_addiction/experiment_0_gemma_corrected/final_gemma_20251004_172426.json', 'r') as f:
        data = json.load(f)
    print(f"Loaded {len(data['results'])} Gemma experiments")
    return data['results']
...
def create_scatter_plot(condition_stats):
    ...
    ax.set_title('Gemma: Composite Index', fontsize=26)
    ...
    ax.set_ylabel('Bankruptcy Rate (%)', fontsize=24)
    ax.set_title('Gemma: I_EV', fontsize=26)
    ax.grid(True, alpha=0.3)
    ...
    ax.set_ylabel('Bankruptcy Rate (%)', fontsize=24)
    ax.set_title('Gemma: I_LC', fontsize=26)
    ax.grid(True, alpha=0.3)
    ...
    ax.set_ylabel('Bankruptcy Rate (%)', fontsize=24)
    ax.set_title('Gemma: I_EB', fontsize=26)
    ax.grid(True, alpha=0.3)

    plt.tight_layout()
    output_path = '/home/ubuntu/llm_addiction/experiment_0_llama_gemma_restart/analysis/figures/gemma/irrationality_metrics.png'
...
def save_csv(condition_stats):
    """Save results to CSV"""
    import csv
    output_path = '/home/ubuntu/llm_addiction/experiment_0_llama_gemma_restart/analysis/figures/gemma/irrationality_metrics.csv'
