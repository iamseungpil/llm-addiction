...
RESULTS=$REPO/experiments/07_sae_readout/results
...
log "Running critic..."
cd $REPO
ALIGNED=$($PYTHON experiments/07_sae_readout/scripts/critic_v7_results.py 'aligned_steering_combined_*.json' 2>&1)
SHARED=$($PYTHON experiments/07_sae_readout/scripts/critic_v7_results.py 'shared_axis_*_202*.json' 2>&1)
echo "$ALIGNED" | head -2 | tee -a "$LOG"
echo "$SHARED" | head -2 | tee -a "$LOG"

# 4. Commit + push if new results
if ! git diff --quiet HEAD -- experiments/07_sae_readout/results/checkpoints experiments/07_sae_readout/results/json 2>/dev/null; then
    git add experiments/07_sae_readout/results/checkpoints experiments/07_sae_readout/results/json 2>/dev/null
...
if echo "$ALIGNED" | grep -q '\[canonical\].*experiment_a/llama/ic' && \
   echo "$ALIGNED" | grep -q '\[canonical\].*experiment_b/gemma/ic'; then
    ...
    if [ "${WAVE2_RUNNING:-0}" -lt 1 ]; then
        ...
        timeout 45 amlt ssh node-recovery-h200-0415 :metacognition_run_c -c "bash /scratch/llm_addiction/experiments/07_sae_readout/scripts/launch_runc_v6_wave2.sh" 2>&1 | tail -5 | tee -a "$LOG"
    else
