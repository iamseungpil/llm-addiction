...
DATA_DIR = Path("/home/v-seungplee/data/llm-addiction/behavioral/slot_machine/llama_v4_role")
OUTPUT_DIR = Path("/home/v-seungplee/data/llm-addiction/sae_features_v3/slot_machine/llama")
...
def phase_a_extract(
    ...
    for i in tqdm(range(n_rounds), desc="Forward passes"):
        ...
        # Progress log every 5000 rounds (no checkpoint save — too slow)
        if i > 0 and i % 5000 == 0:
            logger.info(f"Progress: {i}/{n_rounds} ({i/n_rounds*100:.1f}%), skipped={n_skipped}")
...
def _save_checkpoint(checkpoint_dir, hidden_all, valid_mask, n_rounds, layers,
    ...
    # each requested layer NUMBER to its stored row via the `layers` array
    # (the count-only shape[1]==32 guard cannot tell a post-layer dump from an
    # embedding-row dump) and (b) join i_ba/balance from game_ids/round_nums/
    # balances, and so verify_llama_fullhidden.py can key-align the game-level
    # [8,12,22,25,30] dp-dump rows against these round-level rows. The
    # hidden_states/valid_mask arrays stay byte-identical to the IC-file
    # convention; only NEW provenance keys are added.
    ...
    # Use uncompressed save — 32GB but ~1 min vs compressed ~12 min
