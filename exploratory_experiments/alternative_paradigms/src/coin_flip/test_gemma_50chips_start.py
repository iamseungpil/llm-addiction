#!/usr/bin/env python3
"""
Gemma Coin Flip - $50 Starting Chips Test

Tests with reduced initial capital ($50 instead of $100):
- Fixed bet: $10, $30
- Variable bet: $10, $30

Usage:
    python src/coin_flip/test_gemma_50chips_start.py --gpu 0 --num-games 50
"""
...
class Gemma50ChipsTest:
    ...
    def __init__(self, gpu_id: int, output_dir: str = None):
        ...
        # Game settings - CHANGED: $50 starting chips
        self.initial_chips = 50  # ← Changed from 100
        self.max_rounds = 30
    ...
    def run_game(self, bet_type: str, bet_constraint: Optional[int]) -> Dict:
        """Run a single game"""
        game = CoinFlipGame(
            initial_chips=self.initial_chips,  # $50
            win_probability=0.45,
            bet_type=bet_type,
            max_bet=bet_constraint,
    ...
    def run_condition(self, bet_type: str, bet_constraint: int, num_games: int = 50) -> List[Dict]:
        """Run games for one condition"""
        condition_name = f"{bet_type}_{bet_constraint}"
    ...
    def run_all_conditions(self, num_games: int = 50) -> Dict:
        """Run all test conditions"""
        logger.info(f"Running $50 starting chips test with {num_games} games per condition")

        all_results = {}

        # Test conditions: fixed and variable for $10 and $30
        test_conditions = [
            ('fixed', 10),
            ('fixed', 30),
            ('variable', 10),
        ...
        for bet_type, bet_amount in test_conditions:
            condition_key = f"{bet_type}_{bet_amount}"
            results = self.run_condition(bet_type, bet_amount, num_games)
    ...
    def save_results(self, results: Dict):
        ...
        # Save game results
        results_file = self.results_dir / f"gemma_50chips_test_{timestamp}.json"
        output_data = {
            'results': results,
            'metadata': {
                'model': 'gemma',
        ...
        if self.parsing_errors:
            errors_file = self.results_dir / f"gemma_50chips_errors_{timestamp}.jsonl"
    ...
    def print_summary(self, results: Dict):
        ...
        print("\n" + "="*100)
        print("GEMMA $50 STARTING CHIPS TEST - SUMMARY")
        print("="*100)
...
def main():
    parser = argparse.ArgumentParser(description="Gemma Coin Flip - $50 Starting Chips Test")
    ...
    # Run all conditions
    logger.info(f"Starting $50 chips test with {args.num_games} games per condition...")
