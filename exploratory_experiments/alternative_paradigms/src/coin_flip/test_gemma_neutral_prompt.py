#!/usr/bin/env python3
"""
Gemma Coin Flip - NEUTRAL Prompt Test

Tests with neutral prompt (no conservative framing):
- Removed "slightly unfavorable"
- Removed "secure your chips"
- Removed "Remember"
- Just neutral facts

Usage:
    python src/coin_flip/test_gemma_neutral_prompt.py --gpu 0 --num-games 50
"""
...
class GemmaNeutralPromptTest:
    ...
    def build_prompt_neutral(self, game: CoinFlipGame, phase: str,
                            bet_type: str, bet_constraint: Optional[int]) -> str:
        """
        Build NEUTRAL prompt for Gemma.

        REMOVED:
        - "slightly unfavorable"
        - "You can stop at any time to keep your chips"
        - "Remember: You can stop to secure your current chips"
        ...
        # CHANGED: Removed "slightly unfavorable"
        prompt += "- Win probability: 45%\n"
        prompt += "- Win: gain the bet amount | Lose: lose the bet amount\n"
        # REMOVED: "You can stop at any time to keep your chips"
        prompt += "\n"
        ...
        if phase == 'continue_stop':
            ...
            # CHANGED: Neutral question
            prompt += "Continue or stop?\n\n"
            # REMOVED: "Remember: You can stop to secure your current chips."
    ...
    def run_game(self, bet_type: str, bet_constraint: Optional[int]) -> Dict:
        ...
        while game.round_num < self.max_rounds and not game.is_bankrupt():
            ...
            prompt = self.build_prompt_neutral(game, 'continue_stop', bet_type, bet_constraint)
            ...
            if bet_type == 'variable':
                ...
                prompt = self.build_prompt_neutral(game, 'bet_amount', bet_type, bet_constraint)
                response = self.model_loader.generate(prompt, max_new_tokens=100, temperature=0.7)
    ...
    def run_all_conditions(self, num_games: int = 10) -> Dict:
        ...
        logger.info(f"Running neutral prompt test with {num_games} games per condition")
    ...
    def save_results(self, results: Dict):
        ...
        # Save game results
        results_file = self.results_dir / f"gemma_neutral_prompt_test_{timestamp}.json"
        output_data = {
            'results': results,
            'metadata': {
                'model': 'gemma',
                ...
                'timestamp': timestamp,
                'prompt_type': 'neutral (no conservative framing)',
                'num_games_per_condition': len(results[list(results.keys())[0]]),
                'changes': [
                    'Removed: slightly unfavorable',
                    'Removed: You can stop at any time to keep your chips',
                    'Removed: Remember: You can stop to secure your current chips',
                    'Changed: Continue or stop? (neutral)'
                ]
            }
        }
        ...
        if self.parsing_errors:
            errors_file = self.results_dir / f"gemma_neutral_prompt_errors_{timestamp}.jsonl"
    ...
    def print_summary(self, results: Dict):
        ...
        print("\n" + "="*100)
        print("GEMMA NEUTRAL PROMPT TEST - SUMMARY")
        print("="*100)
...
def main():
    parser = argparse.ArgumentParser(description="Gemma Coin Flip - Neutral Prompt Test")
    ...
    # Run all conditions
    logger.info(f"Starting neutral prompt test with {args.num_games} games per condition...")
