#!/usr/bin/env python3
"""
Experiment 3: Pure Feature Group Patching - Reward Choice Validation
...
- Choice validation with only A and C (gain domain)
"""
...
class PureRewardChoiceExperiment:
    def __init__(self, gpu_id='5', seed: int = 42, n_valid: int = 30, max_attempts: int = 100):
        ...
        self.scales = ['no_patch', 0.1, 0.5, 1.0, 2.0, 5.0]
    ...
    def _build_hook(self, layer: int, feature_ids: List[int], scale: float):
        ...
        def patch_hook(module, inputs, output):
            ...
            with torch.no_grad():
                ...
                orig = feats.index_select(-1, feat_idx)[0, 0, :]
                if isinstance(scale, str) and scale == 'no_patch':
                    patched = orig
                elif scale < 1.0:
                    patched = means_s + scale * (orig - means_s)
                else:
                    adj = scale - 1.0
                    patched = orig + adj * (means_b - orig)
                patched = torch.clamp(patched, min=0.0)
                feats[0, 0, feat_idx] = patched
                mod = sae.decode(feats)
                hidden[:, -1:, :] = mod.to(hidden.dtype)
                del feats, mod, last, orig, patched
...
Please respond with only 1 or 2. Choice: """
    ...
    def run_experiment(self):
        ...
        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        output_file = self.results_dir / f'exp3_pure_reward_choice_gpu{self.gpu_id}_{timestamp}.json'
...
if __name__ == "__main__":
    ...
    exp = PureRewardChoiceExperiment(gpu_id=args.gpu, seed=args.seed, n_valid=args.n_valid)
    exp.run_experiment()
