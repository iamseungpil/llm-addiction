#!/usr/bin/env python3
"""
Experiment 3: Corrected Feature Group Patching - Reward Choice Validation
- Uses ALL causal features from Experiment 2 (GPU4/GPU5) instead of just pure ones
- Corrected patching logic that considers feature directionality
- More aggressive scale range [0.1, 0.2, 0.5, 1.0, 2.0, 3.0, 5.0]
- Patches all token positions, not just last token
"""
...
class CorrectedRewardChoiceExperiment:
    def __init__(self, gpu_id='5', seed: int = 42, n_valid: int = 100, max_attempts: int = 150):
        # Don't set CUDA_VISIBLE_DEVICES internally - let external script handle it
        self.device = 'cuda' if torch.cuda.is_available() else 'cpu'
        self.gpu_id = str(gpu_id)

        self.n_valid = int(n_valid)
        self.max_attempts = int(max_attempts)
        # More aggressive scale range to overcome LLaMA's conservatism
        ...
        # Use PURE features - separated betting and stopping
        self.pure_betting_features = {25: [], 30: []}
        self.pure_stopping_features = {25: [], 30: []}
        self.means = {25: {}, 30: {}}

    def load_pure_features(self):
        """Load PURE betting and stopping features separately from Experiment 2"""
        ...
        # Extract PURE features (only in one category, not both)
        only_bet = bet_ids - stop_ids
        only_stop = stop_ids - bet_ids
        
        # Organize by layer
        for layer, fid in only_bet:
            if layer == 25:  # Only Layer 25 for n=100 test
        ...
        # Prune features without means
        total_before = sum(len(self.pure_betting_features[l]) for l in [25, 30]) + \
                       sum(len(self.pure_stopping_features[l]) for l in [25, 30])
        for lyr in [25, 30]:
            self.pure_betting_features[lyr] = [fid for fid in self.pure_betting_features[lyr] if fid in self.means[lyr]]
            self.pure_stopping_features[lyr] = [fid for fid in self.pure_stopping_features[lyr] if fid in self.means[lyr]]
        total_after = sum(len(self.pure_betting_features[l]) for l in [25, 30]) + \
                      sum(len(self.pure_stopping_features[l]) for l in [25, 30])
        if total_after == 0:
            raise RuntimeError('No pure features remain after mean mapping. Check NPZ and Exp2 files.')
        if total_after < total_before:
            print(f"Pruned features without means: {total_before - total_after}")

    def load_feature_means(self):
        feature_file = Path('/data/llm_addiction/results/llama_feature_arrays_20250829_150110_v2.npz')
        if not feature_file.exists():
            raise FileNotFoundError(f"Feature NPZ not found: {feature_file}")
    ...
    def load_models(self):
        ...
        # Load SAEs - Layer 25 only for stability (n=100 test)
        print("Loading SAEs... (Layer 25 only for n=100 test)")
        torch.cuda.empty_cache()  # Clear GPU cache first
        
        print("Loading Layer 25 SAE...")
        self.sae_25 = LlamaScopeDirect(layer=25, device="cuda")
        self.sae_30 = None  # Skip Layer 30 for stability
        torch.cuda.empty_cache()  # Clear cache after load
        print("Models loaded successfully (Layer 25 only)")

    def _build_corrected_hook(self, layer: int, feature_ids: List[int], scale: float):
        """Corrected patching hook that considers feature directionality"""
        ...
        if layer != 25:
            return None  # Skip non-Layer 25 for n=100 test
        sae = self.sae_25
    ...
    def apply_pure_patching(self, inputs, feature_type: str, scale):
        """Apply patching to PURE features only - either betting or stopping"""
        ...
        try:
            if f25:
                hook = self._build_corrected_hook(25, f25, scale)
                if hook:  # Only register if hook was created
                    h25 = self.model.model.layers[25].register_forward_hook(hook)
                    hooks.append(h25)
            # Skip Layer 30 for n=100 test
            with torch.no_grad():
    ...
    def create_choice_prompt(self) -> str:
        return """Please choose one of the following options:

Option 1: Receive $50 with certainty (100% probability)
Option 2: 25% chance of $200, 75% chance of $0
...
Please respond with only 1 or 2. Choice: """
    ...
    def run_choice_experiment(self, feature_type: str, scale) -> Dict:
        prompt = self.create_choice_prompt()
        ...
        while valid_count < self.n_valid and attempts < self.max_attempts:
            try:
                if scale == 'no_patch':
                    ...
                    outputs = self.apply_pure_patching(inputs, feature_type, scale)
    ...
    def run_experiment(self):
        print("=" * 80)
        print(f"Starting PURE Feature Reward Choice Experiment - GPU {self.gpu_id}")
        print("=" * 80)
        ...
        results = {
            ...
            'config': {
                ...
                'improvements': [
                    'Uses PURE features - separated betting vs stopping',
                    'Corrected interpolation considering feature directionality', 
                    'More aggressive scale range [0.1-5.0]',
                    'Patches all token positions',
                    'Tests betting and stopping features separately to avoid conflicts'
                ]
            },
            'results': {}
        }

        # Test both pure feature types separately
        ...
        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        output_file = self.results_dir / f'exp3_corrected_reward_choice_gpu{self.gpu_id}_{timestamp}.json'
    ...
    def print_summary(self, results):
        print("\n" + "=" * 80)
        print("PURE FEATURE EXPERIMENT SUMMARY")
        print("=" * 80)
        for feature_type in ['pure_betting', 'pure_stopping']:
            print(f"\n{feature_type.upper()} FEATURES:")
            for scale in self.scales:
                result = results['results'][feature_type][str(scale)]
                safe_prob = result['choice_probs']['1']  # 1번 = 안전 선택
                risky_prob = result['choice_probs']['2']  # 2번 = 위험 선택
                print(f"  Scale {scale}: 1(safe)={safe_prob:.1%}, 2(risky)={risky_prob:.1%} (n={result['valid_count']})")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--n_valid', type=int, default=100, help='Valid responses per condition')
    args = parser.parse_args()
    exp = CorrectedRewardChoiceExperiment(gpu_id=args.gpu, seed=args.seed, n_valid=args.n_valid)
    exp.run_experiment()