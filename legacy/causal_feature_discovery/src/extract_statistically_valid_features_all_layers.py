#!/usr/bin/env python3
"""
Extract Statistically Valid SAE Features - ALL LAYERS 25-31
Proper statistical analysis with Cohen's d, t-test, and multiple testing correction
Supports all layers 25-31 with comprehensive analysis
"""
...
def save_multilayer_results(selected_features, all_results):
    ...
    output_npz = f'/data/llm_addiction/results/multilayer_features_{timestamp}.npz'
    np.savez(output_npz, **npz_data)
    
    # Save detailed JSON
    output_json = f'/data/llm_addiction/results/multilayer_analysis_{timestamp}.json'
...
def main():
    """Main function"""
    print("="*80)
    print("MULTI-LAYER SAE FEATURE EXTRACTION (Layers 25-31)")
    ...
    print(f"Selected {len(selected_features)} statistically valid features from layers {available_layers}")
    print(f"(Previous single-layer method found 981 features from layers 25,30 only)")
