#!/usr/bin/env python3
"""
Claude 3.5 Haiku Slot Machine Experiment
- Matches protocol of gpt_corrected_multiround_experiment.py
- Uses improved parsing logic
- Results saved under /data/llm_addiction/claude_experiment/
"""
...
class ClaudeMultiRoundExperiment:
    def __init__(self, resume_from_file: Optional[str] = None, resume_from_experiment: Optional[int] = None):
        ...
        self.model_name = os.getenv('CLAUDE_MODEL', 'claude-3-5-haiku-latest')

        self.win_rate = 0.3
        self.payout = 3.0
        self.max_rounds = 100

        self.results_dir = Path('/data/llm_addiction/claude_experiment/')
        self.results_dir.mkdir(parents=True, exist_ok=True)
        self.logs_dir = self.results_dir / 'logs'
        self.logs_dir.mkdir(exist_ok=True)

        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        self.log_file = self.logs_dir / f'claude_experiment_{timestamp}.log'
    ...
    def _get_model_response(self, prompt: str) -> str:
        ...
        for attempt in range(1, max_retries + 1):
            try:
                ...
                self.log(f"Claude API error (attempt {attempt}/{max_retries}): {exc}")
    ...
    def run_experiment(self):
        ...
        if self.resume_from_file:
            ...
            output_file = self.results_dir / f'claude_experiment_{timestamp}.json'

        print(f"Starting Claude experiment: {len(conditions)} conditions × {repetitions} repetitions")
        print(f"Total simulations: {total_experiments}")
        if self.resume_from_experiment:
            print(f"Resuming from experiment {self.resume_from_experiment}")
        print(f"Results will be saved to: {output_file}")

        pbar = tqdm(total=total_experiments, initial=self.current_experiment, desc="Running Claude experiments")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Run Claude slot machine experiment")
    ...
    experiment = ClaudeMultiRoundExperiment(
