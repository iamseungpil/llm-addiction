#!/usr/bin/env python3
"""
Update Gemini component_effects.png to:
...
def load_gemini_data():
    """Load the latest Gemini experimental data"""
...
def analyze_component_effects(gemini_results):
...
def create_updated_component_figure(component_results):
    ...
    ax1.set_ylim(min(bankruptcy_effects) - 5, max(bankruptcy_effects) + 5)

    # Subplot 2: Game Duration Effects
    ax2 = axes[1]
    ...
    # Set y-axis range for rounds
    ax2.set_ylim(min(rounds_effects) - 1, max(rounds_effects) + 1)

    # Subplot 3: Betting Amount Effects
    ax3 = axes[2]
    ...
    # Set y-axis range for betting amounts
    ax3.set_ylim(min(bet_effects) - 10, max(bet_effects) + 10)

    # Add simplified title
    plt.suptitle('Gemini Component Effects on Risk-Taking Behavior',
    ...
    plt.savefig('/home/ubuntu/llm_addiction/gemini_experiment/results/gemini_component_effects.pdf',
                dpi=300, bbox_inches='tight', format='pdf', facecolor='white', edgecolor='none')
    plt.savefig('/home/ubuntu/llm_addiction/gemini_experiment/results/gemini_component_effects.png',
                dpi=300, bbox_inches='tight', format='png', facecolor='white', edgecolor='none')
    plt.close()

    # Print results
    print("\n" + "="*60)
    print("✅ UPDATED GEMINI COMPONENT EFFECTS RESULTS:")
    print("="*60)
...
def main():
    print("="*80)
    print("UPDATING GEMINI COMPONENT EFFECTS FIGURE")
    print("="*80)

    # Load data
    gemini_results = load_gemini_data()

    # Analyze components
    component_results = analyze_component_effects(gemini_results)

    # Create updated figure
    create_updated_component_figure(component_results)

    print("\n" + "="*80)
    print("✅ GEMINI COMPONENT EFFECTS FIGURE UPDATED!")
