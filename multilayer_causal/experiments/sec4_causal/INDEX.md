...
### W14 §4.3 matched twin-graft ladders — the paper's §4.3 numbers (post-PROGRAM COMPLETE confirmatory wave)
- **Design fix over W10's ±G comparison:** one shared pool of decision states whose base prompts contain NEITHER module; three ladders on it — minusG (no-goal control), plusG (goal module grafted onto every prompt), plusM (reward-max grafted). Removes the "appending a module a prompt already has changes nothing" confound. Primary metric = common dose grid {-3,0,+3} slopes.
- **Gemma = G_SPECIFIC (paper's §4.3 headline):** common-grid slopes plusG +0.04692 / control +0.03581 / plusM +0.02182; primary +G−(+M) contrast +0.02367 (CI excludes 0); survives impute-stop (+0.0242), grid-restricted (+0.0251), drop-top-dose (+0.0352), extreme-removed (+0.0082, still excludes 0). vs-control: plusG +0.0111 excludes 0, plusM −0.0140 excludes 0 — goal widens, reward narrows.
- **LLaMA = non-replication (paper stance):** primary contrast −0.0156 (sign flipped), collider-corrected −0.0124, but the most conservative scorings do NOT exclude 0 (extreme-removed −0.0038, drop-top-dose −0.0163) → reported as non-replication of the goal-specific gain, not as a reward-specific channel.
- **Data:** local `multilayer_causal/results/sec4_w14/` (26 jsonl + `sec4_w14_analysis.json`); HF mirror `experiments/sec4_causal/checkpoints/sec4_w14/` + `experiments/sec4_causal/analysis/sec4_w14_analysis.json` (+ `_robustness_20260710.json`).
- **Paper landing:** §4.3 body numbers (+0.0469/+0.0358/+0.0218, twin +0.0237), Figure 5b (per-ladder all-dose fits +0.051/+0.036/+0.027 with LLaMA inset), appendix `tab:causal-condition-writability`. Figure script: paper repo `scripts/gen_fig_cross_context_write.py`.
