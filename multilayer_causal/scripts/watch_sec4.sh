#!/bin/bash
# Poll the mlc-sec4-p0 wave; exit when terminal (done/failed/killed/paused) or
# when >=30 result cells have landed on HF. Exit 0 = results in; 2 = needs action.
set -uo pipefail
AMLT=~/miniconda3/envs/amlt/bin/amlt
PY=~/miniconda3/envs/metavllm/bin/python
EXP=mlc-sec4-p0-0706
HF=llm-addiction-research/llm-addiction
PREFIX=experiments/sec4_causal/checkpoints/sec4_p0
