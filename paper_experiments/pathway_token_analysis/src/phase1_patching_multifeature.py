#!/usr/bin/env python3
"""
Phase 1: Patching + Multi-Feature Extraction (REVISED)

Core idea: When patching each target feature, record activations for ALL causal features
This creates trial-level variation needed for meaningful correlation analysis

Input: Causal features list
Output: JSONL with {target_feature, patch_condition, prompt_type, trial, response, all_features}
"""
...
class PatchingMultiFeatureExtractor:
    def __init__(self, gpu_id: int, n_trials: int = 30, top_n: int = 2787):
        """
        Args:
        ...
        self.feature_means: dict = {}  # Will hold feature mean lookup
...
Choice: """
    ...
    def load_causal_features(self, causal_file: Path, means_file: Path):
        ...
        # For now, just take first N features
    ...
    def generate_with_patching(
        ...
        try:
            with torch.no_grad():
                ...
                # 3. 🔥 KEY FIX: Forward pass with FULL generated sequence
                #    Hook is STILL ACTIVE, so patching is applied!
    ...
    def run_experiment(self, causal_file: Path, means_file: Path, output_dir: Path, offset: int = 0, limit: int = None):
        ...
        with open(output_file, 'w') as f:
            pbar = tqdm(total=total_trials, desc="Patching experiments")
...
def main():
    ...
    parser.add_argument('--top-n', type=int, default=200, help="Number of top features to patch")
    ...
    LOGGER.info(f"=== Phase 1: Patching + Multi-Feature Extraction ===")
    ...
    output_dir = Path("/data/llm_addiction/experiment_pathway_token_analysis/results/phase1_patching")
