...
This is a codebook frozen during the response period, so it is a robustness probe rather than an independent replication, and we will report it as one. Deleting every expression the goal instruction could have supplied also leaves the contrast positive in 6 of 6 models (+3.1 to +41.9), though at the low end LLaMA +3.1 has overlapping intervals and we do not lean on it. Two limits we hold ourselves to: `illusion_of_control` over-fires in the variable condition, where stake size genuinely is under the model's control, and the variable-minus-fixed contrast is negative in Gemini under all eight instrument variants.

**On the framing.** The abstract does not use "cognitive distortion", though it carries the clinical frame in other words, so the point stands. §2's second clinical axis, Finding 5's heading and the appendix subsection each become distortion-*associated language*, with the quantity labelled as the frequency of expressions drawn from prior gambling research. Our gbSA response, W1, sets out why we think the underlying construct — irrationality rather than a mental state — is still the right one.
...
**Second, where a goal does exist, the extractor's error rate is 4.2%.** We count an extraction as an error when no *goal*, *target* or *aim* appears within 150 characters of the matched number **and** that number is one the prompt itself printed, so it cannot be a target the model set. Of 3,638 goal-condition escalation events, 4.2% meet both conditions. The pipeline reproduces the published figure to one decimal on all four conditions above and on all twelve cells of the appendix table, through the figure's own loader on its 9,600 games; separately, the decision parser — a different instrument — flips 14 of 4,775 and 18 of 7,223 adjudicable decisions (0.293% and 0.249%).

**And the contrast survives a stricter definition.** §2 defines escalation as raising a self-set goal *after meeting it*. Applying that achievement test throughout, API-only so both conditions share one instrument:
...
This is a comparison we would like to have, and it is the right one to ask for: it would separate what is specific to prompting a language model from what appears in human repeated-decision settings generally. We have not run it. Doing it responsibly needs a separate human-subject protocol, ethics approval, recruitment, compensation and human-specific definitions of participation, escalation and stopping — none of which fits a response window, and a partial version would be worse than none.

Two clarifications follow rather than a substitute experiment. No rate we report should be read as comparable to a human prevalence or behavioural rate, and the camera-ready will say so outright; the submitted §2 already keeps the comparison at the level of "the clinically defined diagnostic criteria for pathological gambling" rather than rates. And prior human think-aloud work — a 2023 study coding 57, 47 and 46 instances of gambler's fallacy, near-miss and illusion of control — enters only as qualitative context for the categories, not as a matched control.
...
**First, the arms you name are registered and running.** One prepends a worked example of cautious play with timely stopping, the other one escalating example; both in the variable and fixed modes under the plain baseline, on the two open-weight models, n = 100 per cell at cap \$70, same seeds across arms, directions fixed in advance. Report by 3 August. Until then we do not treat this question as answered.
...
**Scope.** These are artificial negative-EV games. What we claim is condition-dependent risk-taking and decodable signals, not addiction, a fixed disposition, or a mechanism; where the traces resemble the human literature's loss-chasing or control language, we report resemblance rather than a shared cause.
