...
**Game length and exposure.** Modelled directly, in new work for this response: a cause-specific hazard on all six models, each decision a risk set, adjusted for cap, balance, round and prompt and clustered on game id, puts the variable condition's per-decision bankruptcy **hazard ratio against fixed** at 90.6 [44.8, 183.4] across the four API providers. The association survives adjustment, though it does not causally isolate stopping or exposure; the full specification will be in the revised paper. A cumulative-stake curve agrees in four of four cells at all six thresholds (nested, not independent).
...
Three map onto validated gambling-cognition constructs — illusion of control, predictive control, the chasing criterion. The fourth does not, and we would rather say so than let it pass as clinical: goal escalation is a persistence measure. The third column earns its place, enlarging the goal contrast in every model (+18.1 to +77.9 scoped against +5.1 to +46.4 unscoped), and a match inside a negation counts as a mention, so reasoning *against* chasing is not scored as chasing.

What we cannot offer is human-annotated validation, the honest limit here: the expressions are ours and over-fire — "stopping now is the smart decision" scores as a distortion. The check we can offer: under a convergent codebook with no goal category the contrast stays positive in 6 of 6 models (a3Zu, W1).
