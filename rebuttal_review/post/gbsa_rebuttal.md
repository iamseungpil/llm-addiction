...
Thank you for naming the alternative explanations precisely. We make the clinical framing behavioural and test how far role-play, wager size, task understanding, stopping and action structure explain the fixed-variable difference.
...
Stopping is a real difference, and so is declining to play at all: refusing to stake most of the balance in a negative-expected-value game is a reasonable response, and where the fixed arm plays fewer games its lower bankruptcy partly reflects that rather than better choices. The re-betting column below shows the same route inside one experiment. The revision keeps the names *fixed* and *variable*, defines fixed at first use as the environment setting the wager, and reports participation, realised wager and first-loss re-betting beside every bankruptcy figure.
...
| condition (LLaMA-3.1-8B, cap \$70, BASE prompt) | bankruptcy | difference from the row above | re-bets after a first loss | mean rounds |
|---|---|---|---|---|
...
The autoencoder was used to inspect internal representations at the feature level rather than to maximise accuracy, and the top-200 cut bounded compute; 200 is not claimed to be optimal. Because that readout carried a small signal in the expected direction, we did not present it as causal evidence.
