...
**What it shows, and what it does not.** The behavioural axis meets (2) and (3): it moves Gemma monotonically across all seven doses, 0.009 at α = −3 through 0.182 unperturbed to 0.286 at +3, and clears the random band at z = +4.45. On (1) we count it **partially** satisfied — up raises betting and down lowers it, but we have not put an interval on that difference, so the interval clause is unmet. It also transfers across tasks on Gemma with every sign fixed before any trial ran. The fitted readout meets (2), meets (3) at one dose in one direction, is untested on (1), and fails (4) where it moves, parse success falling from 0.80 to 0.34 against our 0.45 gate.
