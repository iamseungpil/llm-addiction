...
At SAE L22, 415 LLaMA features (and 35 Gemma features) show d≥0.3 in both betting conditions with the same sign. The balanced promoting/inhibiting ratio (LLaMA: 213/202) mirrors the universal neuron structure from §2.2.
...
## 5. Implications for NMT Paper §3.2

The paper's §3.2 identifies 112 causal features in LLaMA SM via activation patching, with safe features in L4–L19 and risky features in L24+. This analysis extends that single-model, single-domain finding in three ways:
...
**Correlational evidence only.** All analyses in this report are correlational. Causal validation via activation patching has been performed only in LLaMA SM (paper §3.2). Cross-model and cross-domain causal validation remains pending.
...
1. **Gemma activation patching**: Test whether Gemma's 600 universal BK neurons causally influence BK outcomes, validating cross-model causal generalization.
2. **IC/MW activation patching**: Test causal features from SM in other paradigms.
3. **Permutation null for common features**: Compute expected number of features with d≥0.3 in both bet types under label permutation.
4. **Balanced accuracy for high-BK conditions**: Report balanced accuracy and F1 alongside AUC for LLaMA MW (75.8% BK).
5. **G-prompt mechanism**: Investigate the 16x behavioral gap (Gemma G: 20.75x vs LLaMA G: 1.26x) despite similar directional alignment.
