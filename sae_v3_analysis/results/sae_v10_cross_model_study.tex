...
\medskip\noindent We analyze two transformer models: \textbf{Gemma-2-9B-IT} (Google, 42 layers, 3,584-dim hidden states) and \textbf{LLaMA-3.1-8B-Instruct} (Meta, 32 layers, 4,096-dim hidden states). Both models play three gambling paradigms --- Investment Choice (IC), Slot Machine (SM), and Mystery Wheel (MW) --- under varying prompt conditions (Fixed/Variable betting, Goal/Money/Warning components). Internal representations are analyzed at two levels: \textbf{hidden states} (the full residual-stream activation vector at each layer) and \textbf{SAE features} (sparse interpretable features extracted via Sparse Autoencoders: GemmaScope at 131K features/layer, LlamaScope at 32K features/layer).

\medskip\noindent\textbf{Three research questions} structure the analysis:
\begin{itemize}[nosep]
    \item \textbf{RQ1}: Do Gemma and LLaMA share common BK patterns? (Cross-model universality)
    \item \textbf{RQ2}: Are BK patterns invariant across IC, SM, and MW? (Cross-domain invariance)
    \item \textbf{RQ3}: Are BK patterns invariant across Fixed/Variable and prompt components? (Cross-condition robustness)
\end{itemize}
...
\medskip\noindent\textbf{Key definitions}:
\begin{description}[nosep,leftmargin=1.5em,labelindent=0em]
    \item[Bankruptcy (BK)] A game ends in bankruptcy when the model's balance reaches \$0 through accumulated losses. BK is the primary outcome variable throughout this report.
    \item[Hidden states] The full activation vector at each transformer layer (residual stream). Gemma has 3,584 dimensions across 42 layers; LLaMA has 4,096 dimensions across 32 layers.
    \item[SAE features] Sparse Autoencoder features --- interpretable, sparse decompositions of hidden states. GemmaScope provides 131K features per layer; LlamaScope provides 32K features per layer.
    \item[Decision Point (DP)] The activation state at the model's final decision moment in each game --- the point where the model last chose to bet or stop.
    \item[Cross-domain transfer] Training a BK classifier on one paradigm (e.g., IC) and testing on another (e.g., SM). Success indicates shared BK structure across domains.
    \item[Classification pipeline] StandardScaler $\rightarrow$ PCA(50 components) $\rightarrow$ Logistic Regression (C=1.0, balanced class weights), evaluated with 5-fold stratified cross-validation. AUC (Area Under the ROC Curve) is the primary metric; 200-permutation tests determine statistical significance.
\end{description}
...
V10 resolves three gaps from V9. First, V9 lacked LLaMA MW data (only IC+SM, 2-paradigm). V10 adds LLaMA MW (3,200 games, BK$=2{,}426$), completing the 3-paradigm matrix for both models. Second, V9 showed BK direction cosine $> 0.81$ and 33--37\% shared BK neurons, but never tested whether a classifier trained on one bet type can predict the other. V10 adds the F1 cross-bet-type transfer analysis with 200-permutation tests. Third, V9's LLaMA factor decomposition was limited to IC+SM (69.5\%). V10 extends this to all three paradigms, reaching 75.8\% outcome-significant features.
...
MW best AUC & \textbf{0.966} (L30) & \textbf{0.963} (L16) \\
\bottomrule
\end{tabular}
\end{table}
...
\noindent LLaMA MW's BK rate (75.8\%) is 44$\times$ higher than Gemma MW (1.7\%), yet classification performance remains equivalent ($0.963$ vs $0.966$). BK information is robustly encoded regardless of base rate.
...
\caption{RQ1 Overview. (a) BK prediction AUC: both models $>0.95$. (b) Universal BK neurons: balanced promoting/inhibiting ratio. (c) Factor decomposition: 65--76\% outcome-significant vs $\sim$1\% null.}
\label{fig:fig1}
\end{figure}

\noindent\textbf{Fig.~\ref{fig:fig1} interpretation}: Three independent lines of evidence for RQ1. Panel (a): Gemma and LLaMA achieve nearly identical BK prediction accuracy (AUC difference $< 0.02$ in every paradigm), despite different architectures and training data. Panel (b): both models encode BK through approximately equal numbers of promoting and inhibiting neurons, suggesting a bidirectional ``push-pull'' mechanism. Panel (c): the proportion of outcome-significant features increases from 65\% to 76\% as more paradigms are added, exceeding the permutation null ($\sim$1\%) by $60\times$+.
...
BK classifiers transfer across paradigms with AUC up to $0.932$ (Gemma IC$\rightarrow$MW) and $0.805$ (LLaMA MW$\rightarrow$IC), demonstrating domain-invariant BK structure. Cross-domain transfer trains a BK classifier on one paradigm's features and tests it on another; AUC exceeding chance ($0.5$) with permutation $p<0.05$ indicates shared BK-predictive structure.
...
\caption{Cross-domain transfer AUC heatmaps. (a) Gemma SAE. (b) LLaMA hidden states.}
\label{fig:fig2}
\end{figure}

\noindent\textbf{Fig.~\ref{fig:fig2} interpretation}: Green cells indicate successful transfer (AUC $\gg 0.5$); red cells indicate failure. The MW paradigm serves as a strong ``hub'' in both models --- BK patterns learned from MW transfer well to other domains, and vice versa. IC$\leftrightarrow$SM transfer is consistently weaker, suggesting these two paradigms encode BK through partially distinct mechanisms despite sharing the same underlying outcome.
...
\caption{Cross-bet-type BK transfer. (a) LLaMA HS: all layers $p=0.000$. (b) Gemma SAE: deep layers succeed, shallow layer (L10) fails due to Variable BK$=14$.}
\label{fig:fig3}
\end{figure}

\noindent\textbf{Fig.~\ref{fig:fig3} interpretation}: This is the central finding of V10. Panel (a): in LLaMA, a BK classifier trained on Fixed games predicts Variable BK with AUC $0.74$--$0.87$, and the reverse direction achieves $0.84$--$0.93$ --- all $p=0.000$. The green shaded area shows the gap above chance. Panel (b): Gemma SAE shows the same pattern at deep layers (L30: Fix$\rightarrow$Var $0.902$), though shallow layers fail (L10 NS), consistent with Gemma's extremely low Variable BK count ($n=14$). Both panels demonstrate that Fixed and Variable betting activate the same underlying BK representation.
...
\begin{table}[H]
\centering
\caption{Variable/Fixed Common BK Features (SAE L22, Both Models)}\label{tab:t15}
\footnotesize
\begin{tabular}{lcc}
\toprule
 & LLaMA IC (Var BK$=77$, Fix BK$=65$) & Gemma IC (Var BK$=14$, Fix BK$=158$) \\
\midrule
Common features ($d \ge 0.3$, same sign) & \textbf{415} & \textbf{35} \\
BK-promoting & 213 & 16 \\
...
\subsubsection*{Supplementary: Var/Fix BK Direction Cosine (Both Models, Both Levels)}
...
\caption{Var/Fix BK Direction Cosine. (a) Hidden state level: LLaMA cos$>0.81$ at all layers; Gemma converges from negative to positive (shallow$\rightarrow$deep). (b) SAE feature level: same pattern reproduced. Gemma's lower cosines reflect Variable BK$=14$ sample size.}
\label{fig:fig4}
\end{figure}

\noindent\textbf{Fig.~\ref{fig:fig4} interpretation}: LLaMA IC (orange) shows consistently high cosine ($>0.81$ at HS, $>0.77$ at SAE) across all layers, indicating strong directional convergence between Variable and Fixed BK representations. Gemma IC (blue) shows a shallow-negative $\rightarrow$ deep-positive gradient at both representation levels, converging to $\cos \approx 0.39$ at L30. The Gemma--LLaMA discrepancy is explained by sample size: Gemma Variable BK$=14$ (noisy direction estimate) vs LLaMA Variable BK$=77$ (stable estimate). The deep-layer convergence in Gemma supports the same conclusion: at sufficient processing depth, Variable and Fixed BK directions align.
