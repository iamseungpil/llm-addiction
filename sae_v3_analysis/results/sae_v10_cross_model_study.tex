...
\caption{(a) Cross-model BK classification AUC. (b) Universal BK neurons: balanced promoting/inhibiting ratio.}
\label{fig:fig1}
\end{figure}
...
\caption{(a) Factor decomposition: 65--76\% outcome-significant vs 1\% null. (b) Bet constraint linear mapping.}
\label{fig:fig4}
\end{figure}

\noindent\textbf{Fig.~\ref{fig:fig4} interpretation}: Panel (a) shows that the majority of SAE features encoding BK do so independently of confounding variables. The red dashed line at 1\% represents the permutation null --- actual percentages (65--76\%) exceed it by $60\times$+. Panel (b) demonstrates that the model's internal BK probability increases linearly as the bet constraint rises from \$10 to \$70, confirming that BK representation encodes continuous risk magnitude, not a binary safe/unsafe distinction.
