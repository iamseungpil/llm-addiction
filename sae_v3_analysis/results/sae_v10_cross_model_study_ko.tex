...
V10은 V9의 세 가지 공백을 해결한다. 첫째, V9에는 LLaMA MW 데이터가 없었다 (IC+SM의 2-패러다임만 존재). V10은 LLaMA MW (3,200 게임, BK$=2{,}426$)를 추가하여 두 모델 모두의 3-패러다임 매트릭스를 완성한다. 둘째, V9에서는 BK 방향 코사인(cosine) $> 0.81$ 및 33--37\%의 공유 BK 뉴런을 보였지만, 한 배팅 유형으로 훈련된 분류기가 다른 유형의 BK를 예측할 수 있는지는 테스트하지 않았다. V10은 200회 순열 검정(permutation test)을 포함한 F1 교차 배팅 유형 전이 분석을 추가한다. 셋째, V9의 LLaMA 요인 분해는 IC+SM으로 제한되었다 (69.5\%). V10은 이를 세 패러다임 전체로 확장하여 75.8\%에 도달한다.
...
두 모델 모두 세 패러다임 전반에서 BK 분류 AUC $> 0.95$를 달성하여, BK 정보 내용이 아키텍처에 불변적임을 확립한다.
...
\noindent LLaMA MW의 BK 비율(75.8\%)은 Gemma MW(1.7\%)보다 44배 높지만, 분류 성능은 동등하게 유지된다 ($0.963$ vs $0.966$). BK 정보는 기저 비율(base rate)에 관계없이 견고하게 인코딩된다.
...
\caption{RQ1 개요. (a) BK 예측 AUC: 두 모델 모두 $>0.95$. (b) 보편적 BK 뉴런: 균형 잡힌 촉진/억제 비율. (c) 요인 분해: 결과-유의 65--76\% 대 귀무 $\sim$1\%.}
\label{fig:fig1}
\end{figure}

\noindent\textbf{그림~\ref{fig:fig1} 해석}: RQ1에 대한 세 가지 독립적 증거 라인. 패널 (a): Gemma와 LLaMA는 서로 다른 아키텍처와 훈련 데이터에도 불구하고 거의 동일한 BK 예측 정확도(모든 패러다임에서 AUC 차이 $< 0.02$)를 달성한다. 패널 (b): 두 모델 모두 거의 동일한 수의 촉진 뉴런과 억제 뉴런을 통해 BK를 인코딩하며, 양방향 ``push-pull'' 메커니즘을 시사한다. 패널 (c): 패러다임이 추가될수록 결과-유의 피처의 비율이 65\%에서 76\%로 증가하며, 순열 귀무($\sim$1\%)를 $60\times$+ 초과한다.
...
BK 분류기가 패러다임 간 AUC $0.932$까지 전이되어 (Gemma IC$\rightarrow$MW), 도메인 불변적 BK 구조를 입증한다 (LLaMA MW$\rightarrow$IC에서도 AUC $0.805$). 교차 도메인 전이는 한 패러다임의 피처로 BK 분류기를 훈련하고 다른 패러다임의 데이터에서 테스트한다; 전이 AUC가 우연 수준($0.5$)을 초과하고 순열 $p<0.05$이면 공유 BK 예측 구조를 나타낸다.
...
\noindent 은닉 상태는 대부분의 전이 방향에서 SAE를 능가한다 (Gemma IC$\rightarrow$SM $+0.247$; Gemma SM$\rightarrow$MW $+0.101$; LLaMA SM$\rightarrow$IC $+0.064$). 예외는 LLaMA IC$\rightarrow$SM으로, L25의 SAE ($0.783$)가 L8의 은닉 상태 ($0.577$)를 초과하지만, 이들은 서로 다른 층이다. 일반적인 패턴은 BK 신호가 희소적이지 않고 분포되어 있음을 확인한다: SAE 희소화는 교차 도메인 전이에서 일관성을 잃을 수 있다. 이는 개입(intervention)에 대한 실질적 시사점을 갖는다 --- 개별 SAE 피처를 수정하는 것만으로는 BK 행동을 완전히 제어하기 어려울 수 있으며, 분포된 은닉 상태 표상이 상보적 정보를 전달한다.
...
\caption{교차 배팅 유형 BK 전이. (a) LLaMA HS: 모든 층 $p=0.000$. (b) Gemma SAE: 깊은 층에서 성공, 얕은 층(L10)은 Variable BK$=14$로 인해 실패.}
\label{fig:fig3}
\end{figure}

\noindent\textbf{그림~\ref{fig:fig3} 해석}: 이것이 V10의 핵심 발견이다. 패널 (a): LLaMA에서 고정 게임으로 훈련된 BK 분류기가 변동 BK를 AUC $0.74$--$0.87$로 예측하며, 반대 방향은 $0.84$--$0.93$에 도달한다 --- 모두 $p=0.000$. 녹색 음영 영역은 우연 수준 위의 격차를 보여준다. 패널 (b): Gemma SAE는 깊은 층에서 동일한 패턴을 보이며 (L30: Fix$\rightarrow$Var $0.902$), 얕은 층은 실패한다 (L10 NS). 이는 Gemma의 극히 낮은 Variable BK 수($n=14$)와 일치한다. 두 패널 모두 고정 및 변동 배팅이 동일한 기본 BK 표상을 활성화함을 입증한다.
...
\begin{table}[H]
\centering
\caption{변동/고정 공통 BK 피처 (SAE L22, 두 모델)}\label{tab:t15}
\footnotesize
\begin{tabular}{lcc}
\toprule
 & LLaMA IC (Var BK$=77$, Fix BK$=65$) & Gemma IC (Var BK$=14$, Fix BK$=158$) \\
\midrule
공통 피처 ($d \ge 0.3$, 동일 부호) & \textbf{415} & \textbf{35} \\
BK-촉진 & 213 & 16 \\
BK-억제 & 202 & 19 \\
활성 피처 테스트 수 & 1,056 & 427 \\
\bottomrule
\end{tabular}
\end{table}

\noindent LLaMA는 균형 잡힌 촉진/억제 비율(213/202)을 가진 415개의 공통 피처를 산출한다. Gemma는 극단적인 Variable BK 표본 불균형($n=14$)을 반영하여 35개만 산출한다. 두 모델 모두 보편적 BK 뉴런(2.2절)에서 관찰된 균형 잡힌 촉진/억제 비율을 유지하며, 배팅 조건에 관계없이 BK 인코딩이 피처 수준에서 양방향적임을 확인한다.
...
\subsubsection*{보충: 변동/고정 BK 방향 코사인 (두 모델, 두 수준)}

변동 BK 방향과 고정 BK 방향 간의 방향 코사인(direction cosine)은 보충적 증거를 제공한다. 양의 코사인은 파산 발생 시 두 배팅 유형이 활성화를 동일한 방향으로 밀어냄을 나타낸다.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{figures/v10_fig4_rq3_direction_cosine.png}
\caption{변동/고정 BK 방향 코사인. (a) 은닉 상태 수준: LLaMA cos$>0.81$ (모든 층); Gemma는 음에서 양으로 수렴 (얕은$\rightarrow$깊은). (b) SAE 피처 수준: 동일 패턴 재현. Gemma의 낮은 코사인은 Variable BK$=14$ 표본 크기를 반영.}
\label{fig:fig4}
\end{figure}

\noindent\textbf{그림~\ref{fig:fig4} 해석}: LLaMA IC (주황색)는 모든 층에서 일관되게 높은 코사인(HS에서 $>0.81$, SAE에서 $>0.77$)을 보이며, 변동 및 고정 BK 표상 간의 강한 방향적 수렴을 나타낸다. Gemma IC (파란색)는 두 표상 수준 모두에서 얕은-음 $\rightarrow$ 깊은-양 기울기를 보이며, L30에서 $\cos \approx 0.39$로 수렴한다. Gemma--LLaMA 간 불일치는 표본 크기로 설명된다: Gemma Variable BK$=14$ (노이즈가 큰 방향 추정) 대 LLaMA Variable BK$=77$ (안정적 추정). Gemma의 깊은 층 수렴은 동일한 결론을 뒷받침한다: 충분한 처리 깊이에서 변동 및 고정 BK 방향이 정렬된다.
