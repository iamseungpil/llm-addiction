...
\begin{description}[style=nextline, leftmargin=2em]
    ...
    \item[BK direction vector] The difference between mean BK hidden states and mean Safe hidden states at a given layer. Used in steering experiments (\S\ref{sec:steering}).
\end{description}
...
% 6. ROBUSTNESS
% ============================================================
\section{Robustness Verification}\label{sec:robustness}

This section verifies that the classification results reported in \S2--\S4 are not artifacts of the specific PCA dimensionality or classifier algorithm chosen. Two analyses address this: PCA dimension sensitivity and classifier comparison.

\subsection{PCA Dimension Sensitivity}
...
\begin{table}[H]
\centering
\caption{PCA Dimension Sensitivity: AUC at PCA=50 vs Full (No PCA)}
\begin{tabular}{lccc}
\toprule
\textbf{Dataset} & \textbf{PCA=50} & \textbf{Full} & \textbf{Saturated at 50?} \\
\midrule
...
\begin{table}[H]
\centering
\caption{Classifier Comparison: AUC at PCA=50}
\begin{tabular}{lcccc}
\toprule
\textbf{Dataset} & \textbf{LR} & \textbf{MLP} & \textbf{SVM-RBF} & \textbf{Max Diff} \\
\midrule
...
% 7. CAUSAL VALIDATION
% ============================================================
\section{Causal Validation: BK Direction Steering}\label{sec:steering}
...
\subsection{Neuron-Level Ablation (Null Result)}
...
\subsection{Direction Steering Method}
...
\begin{table}[H]
\centering
\caption{BK Direction Steering (LLaMA L22, SM, $n$=50 per condition)}
\begin{tabular}{ccc}
\toprule
\textbf{Alpha} & \textbf{Stop Rate} & \textbf{BK Rate} \\
\midrule
$-$2.0 & 0.660 & 0.340 \\
$-$1.0 & 0.520 & 0.480 \\
$-$0.5 & 0.580 & 0.420 \\
...
\caption{BK direction steering dose-response. (a) BK rate increases monotonically with $\alpha$ (Spearman $\rho$=0.927, $p$=0.003). (b) Stop rate decreases correspondingly. Dashed line = baseline; dotted line = random direction control at $\alpha$=1.}
\label{fig:steering}
\end{figure}

\textbf{Fig.~5 interpretation}: The dose-response curve confirms that the BK direction vector causally influences gambling behavior. Panel (a): pushing along the BK direction (positive $\alpha$) increases bankruptcy, pushing against it (negative $\alpha$) increases stopping. The relationship is monotonic (Spearman $\rho$=0.927, $p$=0.003). Panel (b): stop rate mirrors this pattern. The random direction control (dotted line) shows high stop rate at a single $\alpha$ value, but only BK direction demonstrates a graded dose-response across 7 $\alpha$ values.

\subsection{Random Direction Control and Caveat}
...
The critical distinction between BK direction steering and random perturbation is the \textbf{dose-response pattern}. BK direction steering produces a monotonic $\alpha$--BK relationship across 7 $\alpha$ values ($\rho$=0.927, $p$=0.003): more positive $\alpha$ yields more bankruptcy. The random direction was tested at only a single $\alpha$ value ($\alpha$=1.0) and therefore cannot demonstrate or refute dose-response. To fully establish direction-specificity, the random direction control must be tested at multiple $\alpha$ values. If random direction steering also produces a monotonic dose-response with comparable slope, then the BK direction may not be uniquely informative. If---as expected---random direction produces a flat or non-monotonic profile, then the dose-response pattern is specific to the BK direction.

\subsection{Bridging Correlational and Causal Evidence}

The steering results connect three levels of evidence:

\begin{enumerate}[nosep]
    \item \textbf{Correlational} (\S2--\S4): BK and Safe states are classifiable (AUC > 0.95), transfer across domains (AUC 0.58--0.93), and share a low-dimensional subspace.
    \item \textbf{Neuron-level causal} (\S7.1): Individual neuron ablation produces no effect, ruling out sparse causal mechanisms.
    \item \textbf{Direction-level causal} (\S7.3): The BK direction vector produces a significant dose-response ($p$=0.003), confirming that the distributed BK representation has causal influence.
\end{enumerate}
...
\subsection{Causal Validation Summary}
...
\begin{enumerate}[leftmargin=2em]
    \item \textbf{Correlational evidence for cross-model and cross-domain claims.} Causal validation via direction steering has been performed only in LLaMA SM (\S7). Cross-model (Gemma) and cross-domain (IC, MW) causal validation remains pending.
...
\begin{enumerate}[leftmargin=2em]
    \item \textbf{Multi-alpha random direction control}: Test random direction at $\alpha \in \{-2, -1, -0.5, 0.5, 1, 2\}$ to confirm that BK direction produces a stronger dose-response than random perturbation.
    \item \textbf{Multi-layer steering}: Simultaneous steering at L22 + L25 + L30 to test whether multi-layer intervention amplifies the dose-response effect.
    \item \textbf{Gemma steering replication}: Apply BK direction steering to Gemma to test cross-model causal generalization.
    \item \textbf{Permutation null for 415 common features}: Compute the expected number of features with $d$$\geq$0.3 in both bet types under label permutation, establishing whether 415 exceeds chance.
\end{enumerate}
