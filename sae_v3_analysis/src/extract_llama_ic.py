#!/usr/bin/env python3
"""
Extract LLaMA IC V2role SAE features (all rounds) — matches Gemma V3 output format exactly.
...
DATA_DIR = Path("/home/jovyan/beomi/llm-addiction-data/investment_choice_v2_role_llama")
OUTPUT_DIR = Path("/home/jovyan/beomi/llm-addiction-data/sae_features_v3/investment_choice/llama")
...
def load_ic_rounds(data_dir: Path, logger: logging.Logger) -> List[RoundRecord]:
    """Load all IC V2role rounds from constraint JSON files."""
    rounds = []
    game_counter = 0

    json_files = sorted(data_dir.glob("llama_investment_c*.json"))
    # Exclude checkpoint files
    json_files = [f for f in json_files if "checkpoint" not in f.name]

    logger.info(f"Found {len(json_files)} constraint files: {[f.name for f in json_files]}")
    ...
    for filepath in json_files:
        ...
        file_constraint = filepath.stem.split("_c")[1].split("_")[0] if "_c" in filepath.stem else "?"
...
def main():
    parser = argparse.ArgumentParser(description="Extract LLaMA IC V2role SAE features")
    ...
    logger.info("=" * 70)
    logger.info("LLaMA IC V2role SAE Feature Extraction")
    logger.info("=" * 70)
    ...
    # Load data
    rounds = load_ic_rounds(data_dir, logger)
    if not rounds:
