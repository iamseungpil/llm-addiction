...
This version matches the canonical V17 / paper_neural_audit.json methodology:
  ...
  - I_LC  = continuous: max(0, (br - p_br) / p_br)         (continuous, [0, ∞))
            if prev round was a loss, else 0.
            Matches §2 paper formal definition + §3 behavioral analysis
            (regenerate_fig2_fig4.py compute_sm_metrics_from_games).
            Voluntary stops have no SAE hidden state extracted (bet=None
            rounds filtered at data layer), consistent with §3.
...
def compute_round_labels(meta, games):
    ...
    for i in range(n):
        ...
        # I_LC: continuous (§2 / §3 formula). Pair hidden state at round t
        # with label = max(0, (r_t - r_{t-1}) / r_{t-1}) if round t-1 was a loss.
        # Otherwise label = 0 (matches §3's "0 contribution if not loss-following").
        if rn >= 1:
            prev_dec = decs[rn - 1]
