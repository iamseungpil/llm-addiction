#!/usr/bin/env python3
"""
Verify data integrity and regenerate 4model figures with REAL data.
"""
...
def load_experiments() -> pd.DataFrame:
    """Load all model experiments and verify data integrity."""
    records = []
    ...
    for model, file_path in MODEL_PATTERNS.items():
        ...
        if results:
            sample = results[0]
            has_game_history = 'game_history' in sample
            has_round_details = 'round_details' in sample
            print(f"  Data structure: game_history={has_game_history}, round_details={has_round_details}")

            # Count unique conditions
            conditions = set()
            for exp in results:
                conditions.add(exp.get('prompt_combo', 'BASE'))
            print(f"  Unique conditions: {len(conditions)} - {sorted(conditions)[:5]}...")
    ...
    print(f"\n{'='*70}")
    print(f"Total loaded: {len(df)} experiments from {df['model'].nunique()} models")
...
def generate_composite_indices(df: pd.DataFrame):
    ...
    print("\n🎨 Generating: 4model_composite_indices_corrected.png")
    ...
    # Verify data per model
    print("\n📊 Data verification:")
    for model in ['gpt4mini', 'gpt41mini', 'gemini25flash', 'claude']:
        subset = metrics_df[metrics_df['model'] == model]
        print(f"  {model}: {len(subset)} experiments")
        print(f"    i_ev range: [{subset['i_ev'].min():.3f}, {subset['i_ev'].max():.3f}]")
        print(f"    i_lc range: [{subset['i_lc'].min():.3f}, {subset['i_lc'].max():.3f}]")
        print(f"    i_eb range: [{subset['i_eb'].min():.3f}, {subset['i_eb'].max():.3f}]")
        print(f"    Unique conditions: {subset['condition'].nunique()}")
...
def generate_components_breakdown(df: pd.DataFrame):
    ...
    print("\n🎨 Generating: 4model_irrationality_components_breakdown_corrected.png")
    ...
    # Verify unique conditions per model
    print("\n📊 Conditions per model:")
    for model in ['gpt4mini', 'gpt41mini', 'gemini25flash', 'claude']:
        subset = metrics_df[metrics_df['model'] == model]
        conditions = sorted(subset['condition'].unique())
        print(f"  {model}: {len(conditions)} conditions - {conditions}")
...
def main():
    print("="*70)
    print("🔍 VERIFYING DATA AND REGENERATING 4MODEL FIGURES")
    print("="*70)

    print("\n📁 Loading experiment data...")
    df = load_experiments()

    print("\n🎯 Generating figures with VERIFIED data...")
    generate_composite_indices(df)
    generate_components_breakdown(df)

    print("\n" + "="*70)
    print("✅ ALL FIGURES REGENERATED WITH REAL DATA")
